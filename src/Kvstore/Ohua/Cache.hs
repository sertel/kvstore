...
loadTableSF db tableId = liftIO $ evalStateT (withStrictness $ loadTable tableId) KVSState{ _storage=db }
...
decryptTableSF d = do
  decrypter <- get
  (r, KVSState{ _decryption=decrypter' }) <- liftIO $ runStateT (withStrictness $ decryptTable d) KVSState{ _decryption=decrypter }
  put decrypter'
  return r
...
decompressTableSF d = do
  decomp <- get
  (r, KVSState{ _decompression=decomp' }) <- liftIO $ runStateT (withStrictness $ decompressTable d) KVSState{ _decompression=decomp }
  put decomp'
  return r
...
encryptTableSF d = do
  encrypter <- get
  (r, KVSState{ _encryption=encrypter' }) <- liftIO $ runStateT (withStrictness $ encryptTable d) KVSState{ _encryption=encrypter }
  put encrypter'
  return r
...
compressTableSF d = do
  comp <- get
  (r, KVSState{ _compression=comp' }) <- liftIO $ runStateT (withStrictness $ compressTable d) KVSState{ _compression=comp }
  put comp'
  return r
