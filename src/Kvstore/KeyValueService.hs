...
execRequestsFuncImp reqs = do
  ...
  -- mapM here actually folds over the cache! (mapM is a sequential for over the state by definition!)
  (_, (KVSState cache' _ _)) <- runStateT (mapM (\e ->
                                              case e of
                                                (Just entry) -> do
  ...
  -- request handling
  (responses, (KVSState _ db'' serde'')) <- liftIO $ runStateT (mapM RH.serve =<< return reqs) $ KVSState cache' db' serde'
